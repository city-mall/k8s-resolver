import { Resolver, ResolverListener, registerResolver, registerDefaultScheme } from "@grpc/grpc-js/build/src/resolver";
import * as dns from "dns";
import * as util from "util";
import { extractAndSelectServiceConfig, ServiceConfig } from "@grpc/grpc-js/build/src/service-config";
...
import { SubchannelAddress, TcpSubchannelAddress } from "@grpc/grpc-js/build/src/subchannel-address";
import { GrpcUri, uriToString, splitHostPort } from "@grpc/grpc-js/build/src/uri-parser";
import { isIPv6, isIPv4 } from "net";
import { ChannelOptions } from "@grpc/grpc-js/build/src/channel-options";
import { BackoffOptions, BackoffTimeout } from "@grpc/grpc-js/build/src/backoff-timeout";
    ...
    if (this.dnsHostname === null) {
      ...
      this.pendingLookupPromise.then(
        (addressList) => {
          ...
          this.backoff.reset();
          this.backoff.stop();
          const ip4Addresses: dns.LookupAddress[] = addressList.filter((addr) => addr.family === 4);
          const ip6Addresses: dns.LookupAddress[] = addressList.filter((addr) => addr.family === 6);
          this.latestLookupResult = mergeArrays(ip6Addresses, ip4Addresses).map((addr) => ({ host: addr.address, port: +this.port! }));
          const allAddressesString: string = "[" + this.latestLookupResult.map((addr) => addr.host + ":" + addr.port).join(",") + "]";
          trace("Resolved addresses for target " + uriToString(this.target) + ": " + allAddressesString);
        ...
        (err) => {
          trace("Resolution error for target " + uriToString(this.target) + ": " + (err as Error).message);
          this.pendingLookupPromise = null;
          this.stopNextResolutionTimer();
          this.listener.onError(this.defaultResolutionError);
        }
      ...
      if (this.isServiceConfigEnabled && this.pendingTxtPromise === null) {
        ...
        this.pendingTxtPromise.then(
          (txtRecord) => {
            ...
            try {
              this.latestServiceConfig = extractAndSelectServiceConfig(txtRecord, this.percentage);
            } catch (err) {
